...
from Original.pairwisemkl.learner.compute_M import *
from Original.pairwisemkl.learner.compute_a_regression import *
from Original.pairwisemkl.learner.optimize_kernel_weights import *
from Original.pairwisemkl.learner.cg_kron_rls import CGKronRLS
...
k_weights_outer = np.empty([10, P]);
k_weights_outer[:] = np.NAN
# Selected value for the regularization parameter \lambda
model = np.empty([10, 1]);
model[:] = np.NAN
# Predicted drug responses
y_pred_outer_vec = np.empty([len(ids[ids_known])]);
y_pred_outer_vec[:] = np.NAN
# Root mean squared error (RMSE) 
rmse_outer = np.empty([10, 1]);
rmse_outer[:] = np.NAN
# Pearson correlation
pearson_outer = np.empty([10, 1]);
pearson_outer[:] = np.NAN
# F1 score
f1_outer = np.empty([10, 1]);
f1_outer[:] = np.NAN
...
for i_out in range(10):
    ...
    learner = CGKronRLS(K1=kd_list_selected,
                        ...
                        maxiter=400)
    # Prediction
    y_pred_outer_vec[test_ids] = learner.predict(kd_list_selected, kc_list_selected,
                                                 [drug_ids_test for i in range(len(w))],
                                                 [cell_ids_test for i in range(len(w))])
